...
class ArucoDetector:
    ...
    def _setup_ros(self):
        ...
        try:
            ...
            self._get_pose_proxy = rospy.ServiceProxy('/dsr01m1013/system/get_current_pose', GetCurrentPose, persistent=True)
    ...
    def calculate_rotation_angle(self, corner_pixels):
        ...
        point1 = points[0]  # 좌상단(Top-Left)
        point2 = points[1]  # 우상단(Top-Right)
    ...
    def publish_center_and_corners(self, marker_id: int,
        ...
        msg_c = PointStamped(header={'stamp': stamp, 'frame_id': "tool"}, point={'x': cx, 'y': cy, 'z': cz})
        self.pub_center_tool.publish(msg_c)
        
        msg_poly = PolygonStamped(header={'stamp': stamp, 'frame_id': "tool"})
        ...
        if center_b_m is not None and corners_b_m_list is not None:
            ...
            cby = -cby
            msg_cb = PointStamped(header={'stamp': stamp, 'frame_id': "base"}, point={'x': cbx, 'y': cby, 'z': cbz})
            self.pub_center_base.publish(msg_cb)
            
            msg_poly_b = PolygonStamped(header={'stamp': stamp, 'frame_id': "base"})
                ...
                for i, corner in enumerate(corners):
                    ...
                    if d_center is not None:
                        try:
                            ...
                            # print(f"[TOOL][ID {marker_id}] Rotation Angle: {rotation_angle:.2f} degrees")
