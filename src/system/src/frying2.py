...
class CupPlacementController:
    ...
    def run_main_logic(self):
        ...
        # 고정 파라미터
        pos_basic   = [200, 400, 500, 0, 180, 180]
        search_tray = [-536, 660, 500, 0, 180, 90]
        move2ATC  = [567.330, -749.240, 419.800, 0, 180, 180]    #ATC 체결하러 이동
        move2ATC2  = [567.330, -649.240, 619.800, 0, 180, 180]
        ...
        atc1      = [567.330, -749.240, 321.040, 0, 180, 180]    #ATC 체결 하강
        atc2      = [567.330, -649.240, 321.040, 0, 180, 180]    #ATC 체결 후 스테이션과 분리
        atc3      = [567.330, -649.240, 619.800, 0, 180, 180]    #스테이션과 분리 후 상승  
        move2cook = [238.190, -650.040, 619.790, 0, 180, 180]    #조리대로 이동

        #조리 모션 수행
        atc4      = [238.190, -650.040, 320.790, 0, 180, 180]     #조리대 스테이션 체결하러 하강
        atc5      = [238.190, -750.040, 320.790, 0, 180, 180]     #조리대 스테이션 체결
        atc6      = [238.190, -750.040, 420.790, 0, 180, 180]     #조리대 atc 분리 후 상승
        point     = [238.190, -650.040, 520.790, 0, 180, 180]     #조리모션 준비 위치
        swing     = [238.190, -650.040, 520.790, 0, 120, -60]     #흔드는 모션
        up        = [238.190, -650.040, 620.790, 0, 180, 180]     #기름 터는 모션 위
        down      = [238.190, -650.040, 520.790, 0, 180, 180]     #기름 터는 모션 아래
    
        move2tray = [-206 + self.Yg0 - y_pos, 660 + self.Xg0 + x_pos, 600, 0 ,180, 90]

        step1 = [-11     + self.Yg0 - y_pos, 660     + self.Xg0 + x_pos, 600,     33, 120, 107]
        step2 = [-195 + self.Yg0 - y_pos, 645.460 + self.Xg0 + x_pos, 400.120, 118.18, 92.60, 178.33]
        step3 = [-195 + self.Yg0 - y_pos, 645.460 + self.Xg0 + x_pos, 175.560, 118.18, 92.60, 178.33]
        step4 = [-204.310  + self.Yg0 - y_pos, 678.380 + self.Xg0 + x_pos, 173.320, 129.53, 57.19, 160.35]
        ...
        try:
            ...
            time.sleep(0.5)
            self.moveline(move2ATC2,  self.vel, self.acc, self.get_scaled_time(3.0)) # ✅
            time.sleep(0.5)
            self.execute_path2()
            time.sleep(0.5)
            ...
            self.moveline(pos_basic,     self.vel, self.acc, self.get_scaled_time(5.0))
            time.sleep(0.5)
